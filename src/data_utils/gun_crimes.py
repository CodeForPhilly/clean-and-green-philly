...
def gun_crimes(primary_featurelayer):
    ...
    x_grid, y_grid = np.linspace(x.min(), x.max(), 1000), np.linspace(y.min(), y.max(), 1000)
    ...
    # Export as raster
    with rasterio.open('tmp/output.tif', 'w', driver='GTiff',
                    height=zz.shape[0],
                    width=zz.shape[1],
    ...
    coord_list = [(x,y) for x,y in zip(primary_featurelayer.gdf['centroid'].x, primary_featurelayer.gdf['centroid'].y)]
    ...
    guncrime_classifier = mapclassify.Percentiles(primary_featurelayer.gdf['guncrime_density'], pct=[50, 75, 90, 95, 99, 100])
    primary_featurelayer.gdf['guncrime_density'] = primary_featurelayer.gdf['guncrime_density'].apply(guncrime_classifier)
    primary_featurelayer.gdf['guncrime_density'] = primary_featurelayer.gdf['guncrime_density'].astype(float)

    primary_featurelayer.gdf['guncrime_density'] = primary_featurelayer.gdf['guncrime_density'].replace([0, 1, 2, 3, 4, 5], ['Bottom 50%', 'Top 50%', 'Top 25%', 'Top 10%', 'Top 5%', 'Top 1%'])
