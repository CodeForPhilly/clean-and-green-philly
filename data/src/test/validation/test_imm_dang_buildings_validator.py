    ...
    return pd.DataFrame(
        {
            ...
            "imm_dang_building": [True, False, True],  # Valid boolean values
...
def test_imm_dang_buildings_validator_schema_edge_cases(base_test_data):
    ...
    # Test with null imm_dang_building values
    test_data = pd.DataFrame(
        {
            "opa_id": base_test_data["opa_id"],
            "imm_dang_building": [
    ...
    # Should fail due to null imm_dang_building values
    assert not result.success
    assert len(result.errors) > 0
    ...
    test_data = pd.DataFrame(
        {
            ...
            "imm_dang_building": [True, False, True],
            "geometry": base_test_data["geometry"],
        }
    )
...
def test_imm_dang_buildings_validator_null_imm_dang_building_values(base_test_data):
    """Test that the validator catches null values in imm_dang_building column."""
    test_data = pd.DataFrame(
        {
            "opa_id": base_test_data["opa_id"],
            "imm_dang_building": [
    ...
    # Should fail due to null values in imm_dang_building column
    assert not result.success
    assert len(result.errors) > 0
