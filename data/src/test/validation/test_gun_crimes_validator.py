...
def sample_gdf(base_test_data):
    """Create a sample GeoDataFrame with valid gun crime KDE data using base test data."""
    # Convert base test data to GeoDataFrame
    gdf = gpd.GeoDataFrame(base_test_data, crs="EPSG:2272")

    # Add gun crime KDE-specific columns with values that pass validation
    gdf["gun_crimes_density"] = [1e-10, 2e-10, 3e-10]
    gdf["gun_crimes_density_zscore"] = [
        0.05,
        -0.05,
        0.0,
    ]  # Mean close to 0, std reasonable
    gdf["gun_crimes_density_percentile"] = [25, 50, 95]  # Include one high-density area
...
def large_sample_gdf(large_test_data):
    ...
    # Add gun crime KDE-specific columns with realistic distributions
    gdf["gun_crimes_density"] = [1e-10 + i * 1e-11 for i in range(len(gdf))]
    gdf["gun_crimes_density_zscore"] = [
...
@pytest.fixture
def validator():
    """Create a GunCrimesOutputValidator instance."""
    return GunCrimesOutputValidator()


def test_density_too_high(validator, sample_gdf):
    """Test that validator catches gun crime density values that are too high."""
    # Set density value above the 1e-8 threshold
    gdf_high_density = sample_gdf.copy()
    gdf_high_density.loc[0, "gun_crimes_density"] = 2e-8  # Above 1e-8 threshold
...
def test_negative_density(validator, sample_gdf):
    """Test that validator catches negative gun crime density values."""
    # Set negative density value
    gdf_negative = sample_gdf.copy()
    gdf_negative.loc[0, "gun_crimes_density"] = -1e-10
...
def test_density_mean_too_high(validator, sample_gdf):
    """Test that validator catches gun crime density mean that is too high."""
    # Set all density values to be very high, making the mean too high
    gdf_high_mean = sample_gdf.copy()
    gdf_high_mean["gun_crimes_density"] = [1e-8, 1e-8, 1e-8]  # All at threshold
...
def test_density_mean_too_low(validator, sample_gdf):
    """Test that validator catches gun crime density mean that is too low."""
    # Set all density values to be very low, making the mean too low
    gdf_low_mean = sample_gdf.copy()
    gdf_low_mean["gun_crimes_density"] = [
        1e-13,
        1e-13,
        1e-13,
...
def test_zscore_mean_too_far_from_zero(validator, sample_gdf):
    ...
    gdf_bad_zscore_mean["gun_crimes_density_zscore"] = [5.0, 5.0, 5.0]  # Mean = 5.0
...
def test_zscore_std_too_low(validator, sample_gdf):
    ...
    gdf_low_zscore_std["gun_crimes_density_zscore"] = [
        0.1,
        0.1,
        0.1,
...
def test_zscore_std_too_high(validator, sample_gdf):
    ...
    gdf_high_zscore_std["gun_crimes_density_zscore"] = [-10.0, 0.0, 10.0]  # High std
...
def test_extreme_zscore_outliers(validator, sample_gdf):
    ...
    gdf_extreme_outliers["gun_crimes_density_zscore"] = [15.0, 0.0, -12.0]  # Beyond ±10
...
def test_high_density_areas_distribution(validator, sample_gdf):
    ...
    gdf_many_high_density["gun_crimes_density_percentile"] = [
        95,
        96,
        97,
    ]  # All in top 10%
...
def test_few_high_density_areas(validator, sample_gdf):
    ...
    gdf_few_high_density["gun_crimes_density_percentile"] = [
        10,
        20,
        30,
    ]  # All in bottom 30%
...
def test_valid_gun_crime_data_passes(validator, large_sample_gdf):
    """Test that valid gun crime data passes all validation."""
...
def test_realistic_gun_crime_ranges(validator, large_sample_gdf):
    """Test that realistic gun crime density and z-score ranges pass validation."""
    # Use realistic values based on the actual statistics provided
    gdf_realistic = large_sample_gdf.copy()
    gdf_realistic["gun_crimes_density"] = [
        1e-10 + i * 1e-11 for i in range(len(gdf_realistic))
    ]  # Realistic density range
    gdf_realistic["gun_crimes_density_zscore"] = [
