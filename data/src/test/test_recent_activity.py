...
def test_recent_activity_success(mock_datetime, mock_get):
    ...
    assert result_feature_layer.gdf.loc[0, "latest_permit_date"] == pd.Timestamp("2024-12-31")
    assert result_feature_layer.gdf.loc[0, "days_since_permit"] == 1
    assert result_feature_layer.gdf.loc[0, "has_permit_record"]

    assert result_feature_layer.gdf.loc[0, "latest_business_license_date"] == pd.Timestamp("2024-12-30")
    assert result_feature_layer.gdf.loc[0, "days_since_business_license"] == 2
    assert result_feature_layer.gdf.loc[0, "has_business_license_record"]
    ...
    assert result_feature_layer.gdf.loc[0, "has_appeal_record"]

    assert len(result_feature_layer.collected_metadata) == 1

@patch("new_etl.data_utils.recent_activity.requests.get", side_effect=mock_requests_get_empty)
@patch("new_etl.data_utils.recent_activity.datetime")
def test_recent_activity_empty_results(mock_datetime, mock_get):
    ...
    assert not result_feature_layer.gdf.loc[0, "has_permit_record"]
    ...
    assert not result_feature_layer.gdf.loc[0, "has_business_license_record"]
    ...
    assert not result_feature_layer.gdf.loc[0, "has_appeal_record"]

@patch("new_etl.data_utils.recent_activity.requests.get", side_effect=mock_requests_get_error)
@patch("new_etl.data_utils.recent_activity.datetime")
...
def test_recent_activity_id_handling(mock_datetime, mock_get):
    mock_datetime.now.return_value = FIXED_TIME
    mock_feature_layer = Mock()
    mock_feature_layer.gdf = pd.DataFrame({"opa_id": ["123", "456", "789-ABC", "00123"]})
    mock_feature_layer.collected_metadata = []
    ...
    def string_based_mock(url, params):
        ...
        if "permitissuedate" in q:
            response.json.return_value = {
                "rows": [
                    {"opa_account_num": "123", "latest_permit_date": "2024-12-31"},
                    {"opa_account_num": "00123", "latest_permit_date": "2024-12-15"},
                ]
            }
        return response
    ...
    assert result_feature_layer.gdf.loc[0, "latest_permit_date"] == pd.Timestamp("2024-12-31")
    assert result_feature_layer.gdf.loc[3, "latest_permit_date"] == pd.Timestamp("2024-12-15")

@patch("new_etl.data_utils.recent_activity.requests.get", side_effect=mock_requests_get_mixed_data)
@patch("new_etl.data_utils.recent_activity.datetime")
def test_recent_activity_multiple_properties(mock_datetime, mock_get):
    ...
    assert result_feature_layer.gdf.loc[1, "latest_permit_date"] == pd.Timestamp("2024-11-15")
    ...
    assert result_feature_layer.gdf.loc[2, "latest_appeal_date"] == pd.Timestamp("2024-09-15")

    assert pd.isna(result_feature_layer.gdf.loc[3, "latest_permit_date"])
    assert result_feature_layer.gdf.loc[3, "latest_business_license_date"] == pd.Timestamp("2024-10-01")
    ...
    assert result_feature_layer.gdf.loc[0, "has_permit_record"]
    assert not result_feature_layer.gdf.loc[1, "has_business_license_record"]
    assert result_feature_layer.gdf.loc[2, "has_appeal_record"]
...
def test_recent_activity_merge_mismatch(mock_datetime, mock_get):
    ...
    assert not gdf.loc[0, "has_permit_record"]
...
def mock_requests_get_missing_column(url, params):
    ...
    q = params.get("q", "")
    if "permitissuedate" in q:
        response.json.return_value = {"rows": [{"opa_account_num": 123}]}
    elif "mostrecentissuedate" in q:
        response.json.return_value = {
            "rows": [{"opa_account_num": 123, "latest_business_license_date": "2024-12-30"}]
        }
    elif "scheduleddate" in q:
        response.json.return_value = {
            "rows": [{"opa_account_num": 123, "latest_appeal_date": "2024-12-29"}]
...
@patch("new_etl.data_utils.recent_activity.requests.get", side_effect=mock_requests_get_missing_column)
@patch("new_etl.data_utils.recent_activity.datetime")
