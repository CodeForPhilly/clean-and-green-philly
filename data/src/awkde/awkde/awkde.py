...
class GaussianKDE(BaseEstimator):
    ...
    def sample(self, n_samples, random_state=None):
        ...
        sample = np.atleast_2d(rndgen.normal(means, 1.0 / invbw))
    ...
    def from_json(cls, fpath, verb=False):
        ...
        kde = cls(glob_bw=d["glob_bw"], alpha=d["alpha"], diag_cov=d["diag_cov"])
        ...
        if len(kde._mean) != kde._n_features:
            raise ValueError(
                "'kde_X_mean' has not the same dimension " + "as the X values."
            )
    ...
    def _get_glob_bw(self, glob_bw):
        ...
        if glob_bw == "silverman":
            return np.power(nsam * (dim + 2.0) / 4.0, -1.0 / (dim + 4))
        elif glob_bw == "scott":
            return np.power(nsam, -1.0 / (dim + 4))
        else:
            return self._glob_bw
