...
class LIComplaintsOutputValidator(BaseKDEValidator):
    """
    Validator for LI Complaints KDE outputs.

    Validates LI complaints density calculations with application-specific ranges:
    - l_and_i_complaints_density: typically very small values (e-10 to e-9 range)
    - l_and_i_complaints_density_zscore: roughly normal distribution around 0, std ~1
    - l_and_i_complaints_density_percentile: 0-100 range (validated by base class)
    """

    @property
    def density_label_column(self) -> str:
        return "l_and_i_complaints_density_label"

    @property
    def density_column(self) -> str:
        return "l_and_i_complaints_density"

    @property
    def density_zscore_column(self) -> str:
        return "l_and_i_complaints_density_zscore"

    @property
    def density_percentile_column(self) -> str:
        return "l_and_i_complaints_density_percentile"

    def _validate_density_ranges(self, gdf: gpd.GeoDataFrame, errors: list):
        """
        Validate LI complaints density and zscore ranges.

        Based on actual observed statistics:
        - density: max ~1.98e-09, mean ~4.62e-10, std ~3.41e-10
        - zscore: max ~4.46, mean ~0.001, std ~1.0
        """
        # Validate density values (LI complaints densities are typically very small)
        if self.density_column in gdf.columns:
            ...
            if max_density > 1e-8:  # Allow up to 1e-8 (much higher than observed max)
                errors.append(
                    f"LI complaints density values appear too high: max={max_density:.2e} "
                    f"(expected < 1e-8)"
                )
            ...
            if min_density < 0:
                errors.append(
                    f"LI complaints density values should be non-negative: min={min_density:.2e}"
                )
            ...
            if mean_density > 1e-8 or mean_density < 1e-12:
                errors.append(
                    f"LI complaints density mean appears outside expected range: "
        ...
        if self.density_zscore_column in gdf.columns:
            ...
            if abs(mean_zscore) > 0.1:  # Allow some deviation from 0
                errors.append(
                    f"LI complaints z-score mean appears too far from 0: "
            ...
            if std_zscore < 0.5 or std_zscore > 2.0:  # Allow reasonable range
                errors.append(
                    f"LI complaints z-score standard deviation appears outside expected range: "
    ...
    def _validate_application_specific(self, gdf: gpd.GeoDataFrame, errors: list):
        """
        Validate LI complaints-specific requirements.
        """
        ...
        if self.density_percentile_column in gdf.columns:
            ...
            if (
                ...
                errors.append(
                    f"LI complaints high-density areas (≥90th percentile) count appears unusual: "
                    f"{high_density_count} out of {total_count} ({high_density_count / total_count * 100:.1f}%) "
                    f"(expected ~10%)"
                )

    def _print_statistical_summary(self, gdf: gpd.GeoDataFrame):
        """Print LI complaints KDE-specific statistical summary."""
        self._print_summary_header("LI Complaints KDE Output Statistics", gdf)

        if self.density_column in gdf.columns:
            density_stats = gdf[self.density_column].describe()
            print("LI Complaints Density statistics:")
        ...
        if self.density_zscore_column in gdf.columns:
            ...
            print("LI Complaints Z-score statistics:")
        ...
        if self.density_percentile_column in gdf.columns:
            ...
            print("LI Complaints Percentile statistics:")
